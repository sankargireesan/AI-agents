...
def enhancedFeatureExtractorDigit(datum):
    ...
    if count < (.65*total/2) and count > (.35*total/2):
        features['left-symm'] = True
    else:
        features['left-symm'] = False
    ...
    if count < (.65*total/2) and count > (.35*total/2):
        features['upper-symm'] = True
    else:
        features['upper-symm'] = False

    if count >= total/2:
        features['upper'] = True
    else:
        features['upper'] = False
    ...
    # if features['upper-symm']:
    #     features['1']= False
    #     features['2']= False
    #
    # if features['left-symm']:
    #     features['3']= False
    #     features['4']= False
    #
    count = 0
    for x in range(DIGIT_DATUM_WIDTH/2):
        for y in range(3*DIGIT_DATUM_HEIGHT/8,DIGIT_DATUM_HEIGHT/2):
            if features[(x,y)] == 1:
                count += 1
    if count < .25*DIGIT_DATUM_HEIGHT*DIGIT_DATUM_WIDTH/16:
    ...
    for x in range(3*DIGIT_DATUM_WIDTH/8,5*DIGIT_DATUM_WIDTH/8):
        for y in range(3*DIGIT_DATUM_HEIGHT/8,5*DIGIT_DATUM_HEIGHT/8):
            if features[(x,y)] == 1:
                count += 1

    if count < .1*DIGIT_DATUM_HEIGHT*DIGIT_DATUM_WIDTH/32:
        features['zero']=True
    else:
        features['zero']=False

    if total > .1*DIGIT_DATUM_HEIGHT*DIGIT_DATUM_WIDTH:
        features['maj']=True
    else:
        features['maj']=False
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        ...
        for y in range(DIGIT_DATUM_HEIGHT):
            if features[(x,y)] == 1:
                t +=1
        if t > .6*DIGIT_DATUM_HEIGHT:
            l +=1

    if l>0:
        features['lines']=True
    else:
        features['lines']=False
    # if l ==2:
    #     features['2lines']=True
    # else:
    #     features['2lines']=False
    ...
    for y in range(DIGIT_DATUM_HEIGHT):
        ...
        for x in range(DIGIT_DATUM_WIDTH/4):
            if features[(x,y)] == 1:
                t +=1
        if t > .4*DIGIT_DATUM_WIDTH:
            l +=1

    if l>0:
        features['Hlines']=True
    else:
        features['Hlines']=False


    l = 0
    for y in range(DIGIT_DATUM_HEIGHT):
        t = 0
        for x in range(3*DIGIT_DATUM_WIDTH/8,6*DIGIT_DATUM_WIDTH/8):
            if features[(x,y)] == 1:
                t +=1
        if t > .4*DIGIT_DATUM_WIDTH:
            l +=1
...
def enhancedPacmanFeatures(state, action):
    ...
    pacmanpos = next_state.getPacmanPosition()

    ghostdis =[]
    for ghost in next_state.getGhostPositions():
        ghostdis.append(util.manhattanDistance(ghost, pacmanpos))
    features['ghost'] = 1/(min(ghostdis)+1)
    ...
    foodDistance = []
    for f in foodPos:
        foodDistance.append(util.manhattanDistance(f, pacmanpos))

    if foodDistance:
        features['food'] = min(foodDistance)
    else:
        features['food']=0

    features['len-food'] = len(foodDistance)
