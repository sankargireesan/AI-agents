...
def cornersHeuristic(state, problem):
    ...
    while unexploredCorners:
        ...
        for corner in unexploredCorners:
            temp.append([util.manhattanDistance(node, corner), (corner)])
##            print corner
##            print temp
        distance, node =  min(temp)
##        print corner
##        print unexploredCorners
        total += distance
##        node = corner
##        print node
        unexploredCorners.remove(node)
...
##    temp2=foodGrid.copy()
##    
##    cl = ClosestDotSearchAgent('depthFirstSearch','PositionSearchProblem','nullHeuristic')
##    direction = []
##    nextPathSegment = c1.findPathToClosestDot(currentState)
##    #direction = cl.findPathToClosestDot(state)
##    x,y = position
##    for d in nextPathSegment:        
##        dx, dy = Actions.directionToVector(direction)
##        nextx, nexty = int(x + dx), int(y + dy)
##        x,y=nextx, nexty
##
###    temp2[x][y]=False
##    d=util.manhattanDistance((x,y), position)
##    distance =0
##    food =position
##    if len(temp) > 1:
##        distance, food = min(t)
